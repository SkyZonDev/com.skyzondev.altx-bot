...
// Initialisez le logger avec le chemin du fichier de logs
...
module.exports = {
    ...
    async execute({ interaction }) {
        ...
        const embed_log = new EmbedBuilder()
            ...
            .setColor("#f00000")
            .setFooter({
                text: interaction.user.username, // Pseudo de celui qui ban
                iconURL: interaction.user.displayAvatarURL({ dynamic: true }), // Avatar de celui qui ban
            })
            .setTimestamp();

        const embed_mp = new EmbedBuilder()
            .setTitle("Vous avez été banni de : \nALTX")
        ...
        collector.on('collect', async i => {
            if (i.customId === 'confirmButton') {
                ...
                await interaction.editReply(`L'utilisateur ${user.tag} a été banni pour la raison : ${reason}`);
                suppMsg(interaction);
            } else if (i.customId === 'cancelButton') {
                await interaction.editReply('Bannissement a été annulée.');
                suppMsg(interaction);
            }
        });
